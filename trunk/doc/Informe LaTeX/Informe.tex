...
Vigenere(string clave);    // constructor

La clase tendría como miembro dicha clave.

También existe un constructor alternativo para poder emplear alfabetos de menor tamaño.
...
La encriptación manejaría strings, recibiría en mensaje y lo cifraría devolviendo el correspondiente criptograma.
...
De esta forma haríamos que se desentienda del origen del objeto que está manejando, que podría ser desde memoria o desde disco.

Si necesitáramos descifrar el mensaje entonces se usaría el siguiente método, que resulta similar al anterior:
...
En la otra situación debería invocarse previamente a cada invocación a los métodos de cifrado, un método para reinicializar dichos contadores:
...
Por lo tanto el objeto final estaría constituído por los siguientes miembros privados:
...
A continuación se agrega el mismo gráfico pero acotado a los valores imprimibles.

Como se puede observar el carácter con mayor frecuencia es el 32, que resulta ser el espacio, seguido del carácter e (101), el carácter a(97) y por último el carácter o(111). Estos son los más frecuentes y dependiendo del archivo la frecuencia puede variar levemente.
...
Es objeto contendrá un índice con las posiciones de las cadenas encontradas. Esto se resolverá mediante una lista de objetos patrones, que se definen más adelante.

También sería necesario resolver el cálculo del máximo común divisor. Sólo consiste en aplicar el mcd sobre todas las distancias halladas en todos los patrones y devolver un único resultado.

Finalmente armar un vector de analizadores de frecuencias (un objeto) que contendrá las frecuencias de los elementos encontrados.

Finalmente como resultado final el método devuelve la clave.
...
Contiene un conjunto de caracteres que se repite a lo largo del texto a descrifrar. También contiene las distancias entre dos conjuntos sucesivos de un mismo patrón.
...
También contiene un conjunto de métodos:
...
Objeto: Analizador de frecuencias

Contiene un conjunto de frecuencias de todos los elementos a analizar. Así como también los métodos pertinentes para ser empleados en el análisis de frecuencias.
...
Utilizacion de campos finitos en el algoritmo de RSA:
...
Se toma cada carácter del string y se lo convierte en número entero.
...
Desencriptar recibe el string serializado y encriptado y procede a desencriptar cada carácter del string casteado a un numero grande.

Se realizan las operaciones de campos finitos y se devuelve un numero entero.

Cada número entero (carácter desencriptado) se carga en un string. Al final se devuelve este string desencriptado que es el que el deserializar procede a hidratar.
